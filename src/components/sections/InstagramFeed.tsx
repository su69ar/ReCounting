...
export function InstagramFeed({
  ...
  useGSAP(() => {
    ...
    // Auto sliding layout logic
    const scroller = containerRef.current.querySelector('.flex.overflow-x-auto');
    if (!scroller || posts.length < 2) return;
    ...
    const startAutoSlide = () => {
      autoSlideInterval = setInterval(() => {
        ...
        let nextScroll = scroller.scrollLeft + 300; // Approx one card width
    ...
    scroller.addEventListener('mouseenter', stopAutoSlide);
    scroller.addEventListener('mouseleave', startAutoSlide);
    scroller.addEventListener('touchstart', stopAutoSlide);
    scroller.addEventListener('touchend', startAutoSlide);

    return () => {
      stopAutoSlide();
      scroller.removeEventListener('mouseenter', stopAutoSlide);
      scroller.removeEventListener('mouseleave', startAutoSlide);
      scroller.removeEventListener('touchstart', stopAutoSlide);
      scroller.removeEventListener('touchend', startAutoSlide);
    };
  ...
  return (
    <div ref={containerRef} className={cn("w-full relative max-w-7xl mx-auto", className)}>
      <div className="flex overflow-x-auto gap-4 pb-6 snap-x snap-mandatory scrollbar-hide px-4 md:px-0 scroll-smooth">
        {posts.slice(0, limit).map((post) => (
          <div key={post.id} className="min-w-[200px] md:min-w-[240px] max-w-[280px] shrink-0 snap-start">
            <InstagramPost
              post={post}
              featured={false}
            />
...
function InstagramPost({ post, featured = false }: InstagramPostProps) {
  ...
  return (
    <a
      ...
      className={cn(
        "ig-post group relative block overflow-hidden rounded-xl bg-neutral-100 w-full aspect-square"
      )}
...
function InstagramFeedSkeleton() {
  return (
    <div className="flex overflow-x-auto gap-4 pb-4 scrollbar-hide px-4 md:px-0">
      {Array.from({ length: 4 }).map((_, i) => (
        <div
          key={i}
          className="min-w-[260px] md:min-w-[320px] aspect-square rounded-xl bg-neutral-200 animate-pulse shrink-0"
        />
      ))}
    </div>
  );
