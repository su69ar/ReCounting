...
while [ "${attempt}" -le "${MAX_ATTEMPTS}" ]; do
  ...
  sitemap_headers="$(fetch_head "${SITE_URL}${SITEMAP_PATH}" | tr -d '\r')"
  http_sitemap_headers="$(fetch_head "http://${SITE_HOST}${SITEMAP_PATH}" | tr -d '\r')"
  ...
  if grep -q '<link rel="canonical" href="https://recounting.my.id/"' <<<"${home_html}" &&
    ...
    grep -q "Sitemap: https://recounting.my.id${SITEMAP_PATH}" <<<"${robots_txt}" &&
    ...
    grep -Eiq "^location: https://recounting\\.my\\.id${SITEMAP_PATH}/?$" <<<"${http_sitemap_headers}"; then
